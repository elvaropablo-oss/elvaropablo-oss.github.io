...
const eligible=()=>cfg.offers.filter(o=>o&&o.url&&o.title&&matches(o));
...
function style(){
  ...
  .he-affiliate__head h3{font-size:1rem;margin:0}.he-affiliate__disclosure{font-size:.76rem;opacity:.78;margin:0}
...
function render(){
  ...
  section.innerHTML=`<div class="he-affiliate__head"><h3>${esc(cfg.heading||'Opciones relacionadas')}</h3><p class="he-affiliate__disclosure">Algunos enlaces pueden ser de afiliado: podemos recibir una comisión sin coste extra para ti.</p></div><div class="he-affiliate__grid">${offers.map(o=>`<a class="he-affiliate__item" data-affiliate-link="1" data-offer-id="${esc(o.id||'')}" href="${esc(o.url)}" target="_blank" rel="sponsored noopener"><strong>${esc(o.title)}</strong>${o.description?`<span>${esc(o.description)}</span>`:''}<em>${esc(o.cta||'Ver opción')} →</em></a>`).join('')}</div>`;
